  ...
  switch(
    ...
    "esDemand" = {
      ...
      dt[region == "EU-12" & subsector == "HSR", value := na.approx(value, x=year),
                  by=c("region", "sector", "subsector", "GCAM_technology")]
      dt <- dt[year <= 2010 & !(subsector %in% c("road","LDV","bus","4W","2W"))]
      dt <- dt[GCAM_technology != "Coal"]

      ## missing electric trains
      miss <- CJ(region=dt$region, year=dt$year, sector="trn_freight", Units="million ton-km",
                 subsector="Freight Rail", GCAM_technology=c("Liquids", "Electric"),
                 unique=TRUE)
      fr_train <- dt[miss, on=c("region", "year", "sector", "subsector", "GCAM_technology", "Units")]
